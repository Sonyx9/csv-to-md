...
will be with Markdown formatting).
        ...
        while True:
            addYAML = input(
                'Do you want to have all the entries as frontmatter YAML metadata? Enter "y" for yes and "n" for no: '
            )
        ...
        while True:
            fileNameLength = input(
                "How long should the file name be at maximum? Please enter a number. This excludes the .md ending. "
            )
...
"""
            ...
            if len(fileNameCol) > 1:
                fileNameColSeparator = input(
                    "Please enter the separator (including spaces; any illegal characters will be removed) "
                )
                self.current_settings["fileNameColSeparator"] = fileNameColSeparator
            break

        # NEW: output mode (default multiple for backwards compatibility)
        while True:
            print(
                """
How should the output be generated?
1 = One markdown file (table / NotebookLM)
2 = One markdown file per CSV row (default)
"""
            )
            mode = input("Enter 1 or 2: ").strip()
            if mode == "1":
                self.current_settings["outputMode"] = "single"
                break
            if mode == "2" or mode == "":
                self.current_settings["outputMode"] = "multiple"
                break
            print("You didn't enter 1 or 2")

        # NEW: single file format (only relevant for outputMode=single)
        while True:
            print(
                """
Single file output format?
1 = Markdown table
2 = NotebookLM (heading + bullets)
"""
            )
            fmt = input("Enter 1 or 2 (default 1): ").strip()
            if fmt == "" or fmt == "1":
                self.current_settings["singleFileFormat"] = "table"
                break
            if fmt == "2":
                self.current_settings["singleFileFormat"] = "notebooklm"
                break
            print("You didn't enter 1 or 2")

        # to this the formattings of each column will be added (multi-file mode)
